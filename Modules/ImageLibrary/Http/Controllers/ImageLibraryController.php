...
class ImageLibraryController extends Controller
{
    ...
    public function store(Request $request)
    {
        $this->validate($request, [
            ...
            'filename' => 'image|required|mimes:jpeg,png,jpg,gif,svg|max:2048'
         ]);


        $originalImage  = $request->file('filename');
        $thumbnailImage = Image::make($originalImage);
        $thumbnailPath  = public_path().'/images/media/thumbnail/';
        $originalPath   = public_path().'/images/media/images/';
        ...
        $photo_galery->file_name = $newName;
